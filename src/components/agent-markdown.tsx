...
function hasMarkdownTable(text: string) {
  ...
  return lines.some((line, index) => {
    ...
    return Boolean(
      next && /^\s*\|?\s*:?-{3,}:?\s*(\|\s*:?-{3,}:?\s*)+\|?\s*$/.test(next)
    )
  })
}
...
function unwrapMarkdownTableFences(text: string) {
  return text.replace(
    ...
    (match, prefix: string, body: string) =>
      hasMarkdownTable(body) ? `${prefix}${body.trim()}` : match
  )
}
    ...
    <div className="orrery-code-block">
      <div className="orrery-code-block__bar">
        ...
        <Button
          ...
          variant="ghost"
          size="icon-xs"
          className="h-6 w-6 text-term-dim hover:text-term-name"
...
export function AgentMarkdown({
  ...
  const normalized = useMemo(
    () => preprocessAgentMarkdown(text, streaming),
    [streaming, text]
  )
