import { ArrowUp, Braces, GitBranch, Paperclip, RefreshCw, Square, Terminal, TriangleAlert } from 'lucide-react';
...
export function ChatDetail({ core, layout, composer, newChat, terminal, actions, interactions, diff, showRawEvents, setShowRawEvents }: ChatDetailProps) {
  const {
    runtimeClient,
    isRuntimeAvailable,
    isElectron,
        ...
        >
          ...
          <div className="flex items-center gap-1 px-2 pb-2 pt-1">
            ...
            <div className="ml-auto flex items-center gap-2">
              {composerHasPayload && !canKill ? <span className="hidden font-mono text-[10px] text-term-faint @[26rem]:inline">{sendShortcutHint}</span> : null}
              {canKill ? (
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Button
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Button
                      className="size-8 shrink-0 rounded-full"
                      size="icon-sm"
                      disabled={!isRuntimeAvailable || (selectedSession ? !canResume || isResuming : isCreating || !newCwdValidation.ok) || !composerHasPayload}
