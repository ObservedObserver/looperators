import { Check, ClipboardCheck, Send, Square, X } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { type RuntimeRequestDecision, type RuntimeRequest, type UserInputAnswerValue, type UserInputRequest } from '@/shared/provider-runtime';
...
export type RuntimeInteractionPanelProps = {
  ...
  onRespond: (request: RuntimeRequest, decision: RuntimeRequestDecision) => void;
  ...
  return (
    <div className="shrink-0 border-b border-ink-line bg-ink px-3.5 py-3">
      <div className="mb-2 flex items-center gap-2 font-mono">
        <span className="text-[10px] uppercase tracking-[0.16em] text-term-amber">Action needed</span>
      ...
      <div className="space-y-2">
        {requests.map((request) => {
          ...
          return (
            <div key={request.id} className="rounded-lg border border-term-amber/35 bg-term-amber/10 p-3 font-mono">
              <div className="flex min-w-0 items-start gap-2">
                <span className="pt-0.5 text-term-amber">?</span>
                <div className="min-w-0 flex-1">
                  <div className="truncate text-[12.5px] font-medium text-term-name">{request.title}</div>
                  {request.body ? (
                    <p className="mt-1 max-h-24 overflow-y-auto whitespace-pre-wrap break-words text-[11.5px] leading-5 text-term-dim">{request.body}</p>
                  ) : null}
                  <div className="mt-1 text-[10.5px] uppercase tracking-[0.08em] text-term-faint">
                    {requestKindLabels[request.kind]} · {formatClock(request.createdAt)}
                  </div>
                </div>
              </div>
        ...
        {userInputRequests.map((request) => {
          ...
          return (
            <div key={request.id} className="rounded-lg border border-term-cyan/35 bg-term-cyan/10 p-3 font-mono">
              ...
              <p className="mt-1 max-h-24 overflow-y-auto whitespace-pre-wrap break-words text-[11.5px] leading-5 text-term-dim">{request.prompt}</p>
              {hasStructuredQuestions ? (
                  ...
                  {questions.map((question) => {
                    ...
                    return (
                      <div key={question.id} className="rounded-md border border-ink-line/80 bg-ink/70 p-2.5">
                        ...
                        <div className="mt-1 whitespace-pre-wrap break-words text-[12px] leading-5 text-term-name">{question.label}</div>
                        {question.isSecret ? (
                          <div className="mt-1 text-[10.5px] leading-4 text-term-amber">Secret input requested; the answer is stored in session history.</div>
                        ) : null}
                        {question.options?.length ? (
                          <div className="mt-2 space-y-1.5">
                            {question.options.map((option) => {
                              const checked = question.multiSelect ? optionValues.includes(option.id) : questionDraft === option.id;
                              return (
                                <label
                                  key={option.id}
                                ...
                                >
                                  <input
                                    ...
                                    onChange={() => {
                                      if (question.multiSelect) {
                                        const next = checked ? optionValues.filter((item) => item !== option.id) : [...optionValues, option.id];
                                  ...
                                  />
                                  <span>
                                    <span className="block text-term-name">{option.label}</span>
                                    {option.description ? <span className="block text-term-faint">{option.description}</span> : null}
                                  </span>
                                </label>
                              );
                            })}
                ...
                <textarea
                  ...
                  disabled={isPending}
                  onChange={(event) => onDraftChange(userInputDraftKey(request), event.target.value)}
                />
              )}
              <Button
