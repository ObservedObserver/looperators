...
export function WorkflowStep({ index, title, detail, status }: { index: number; title: string; detail: string; status: WorkflowStepStatus }) {
  return (
    <div className={cn('grid grid-cols-[28px_minmax(0,1fr)] gap-2 rounded-lg border px-2.5 py-2 font-mono', workflowStepClassName(status))}>
      ...
      <span className="min-w-0">
        ...
        <span className="mt-0.5 block line-clamp-2 text-[10.5px] leading-4 opacity-75">{detail}</span>
      </span>
    </div>
  );
}

export function WorkflowSummaryRow({ label, children }: { label: string; children: ReactNode }) {
  return (
