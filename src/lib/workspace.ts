import { type AgentSession, type ProjectContext } from '@/shared/graph-state';
import { getActiveRuntimeClient } from '@/runtime-client';
...
export function latestSessionCwd(sessions: AgentSession[], invalidCwds = new Set<string>()) {
  const latestSession =
...
export function branchCandidatesForSession(session: AgentSession) {
  ...
  return uniqueStrings([project.baseBranch, project.workMode === 'local' ? project.branch : undefined]);
}
...
export function projectOptionsFromSessions(
  ...
  addProject({
    ...
    branches: uniqueStrings([projectContext?.currentBranch, ...(projectContext?.branches ?? [])]),
  ...
  sessions.forEach((session) => {
    ...
    addProject({
      ...
      currentBranch: session.project?.workMode === 'local' ? session.project.branch : undefined,
