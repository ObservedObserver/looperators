import { type Report } from '@/shared/graph-state';

export function latestReportForSession(reports: Report[], sessionId: string) {
  return reports.filter((report) => report.from === sessionId).sort((left, right) => right.envelope.ts.localeCompare(left.envelope.ts))[0];
}
...
export function reportBody(report: Report) {
  if (report.payload.type === 'verdict') {
    ...
    if (report.payload.issues?.length) {
      return report.payload.issues.map((issue) => (issue.file ? `${issue.message} (${issue.file})` : issue.message)).join('\n');
    }
