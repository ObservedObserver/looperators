import { type WorkingTreeDiffResult } from '@/shared/graph-state';
...
export function diffRangeLabel(diff: WorkingTreeDiffResult) {
  ...
  const from = diff.range.fromTurnCount !== undefined ? `turn ${diff.range.fromTurnCount}` : diff.range.fromCheckpointRef;
  const to = diff.range.toTurnCount !== undefined ? `turn ${diff.range.toTurnCount}` : diff.range.toCheckpointRef;
  return `${from} -> ${to}`;
}
