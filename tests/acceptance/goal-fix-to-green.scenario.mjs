...
export const description =
  'L3 acceptance (proposal §L3 acceptance): on a real repo, a "fix until npm test is green, at most 4 laps" goal loop — lap 1 the judge runs the suite and reports fail with the failing cases, the retry edge wakes the coder with the issues, lap 2 the judge reports done and the ring stops itself; the scenario then runs npm test deterministically to confirm green. The judge reports through the membrane, which is mounted for the Claude Agent SDK (MCP handoff) and Codex (per-thread config.mcp_servers) alike — runs under either --provider.'
